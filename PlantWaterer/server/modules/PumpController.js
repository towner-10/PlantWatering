...
class PumpController {
    ...
    squirt() {
        if (this.__activated && this.enabled && !this.halted) {
            ...
            this.originalPumpMillis = this.timesPumped;
            ...
            setTimeout(function() {
                ...
                db.addPumpHistory(Format.convertDateToTimestamp(this.startTime), Format.convertDateToTimestamp(endTime));
    ...
    check() {
        ...
        if (this.error > this.tolerance) {
            ...
            if (this.__activated && Date.now() - this.startTime > PUMP_MIN_MILLIS) {
    ...
    */
    setCurrent(current) {
        ...
        if (current > this.target && this.__activated && Date.now() - this.startTime > PUMP_MIN_MILLIS) {
            console.log("Plant has enough water!");
            this.pump.setIntensity(0);
            this.__activated = false;
        }
